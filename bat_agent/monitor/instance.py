...
# If ../bat_agent/__init__.py exists, add ../ to Python search path, so that
...
class InstanceMonitor(batMonitorDaemon):
    def __init__(self, pidfile, instance_configuration_json=None, cfg=None):
        ...
        if instance_configuration_json is not None:
            ...
            for proc in procs:
                ...
                if len(p.cmdline()) > 2 and p.cmdline()[2] == '-name' and p.cmdline()[3] == self._instance_name:
    ...
    def run(self):
        ...
        LOGGER.info("Running self._target:%s" %self._target)
