...
class AgentListener:
    def __init__(self, cfg, hostname=None):
        ...
        self._listener_cfg = cfg[Constant.CONFIG_AGENT_LISTENER_SECTION]
        self._closing = None
        self._connection = None
        self._channel = None
        self._consumer_tag = None
        if hostname is None:
            self.ROUTING_KEY = self._listener_cfg['routing_key']
        else:
            self.ROUTING_KEY = self._listener_cfg['routing_key_raw'].replace("[]", hostname)
        self.EXCHANGE = self._listener_cfg['exchange']
        self.EXCHANGE_TYPE = "topic"
        self.QUEUE = self._listener_cfg['queue']
    ...
    def signal_term_handler(self, signal, frame):
        print 'got sinfterm'
        self.stop_rplistener()

    """ def __startlistiner(self):
     self._connection = self.connect()
     self.open_channel(self._connection)
     self._channel.exchange_declare(exchange=self.EXCHANGE, type='topic')

     result = self._channel.queue_declare(exclusive=True)
     queue_name = result.method.queue

     logger.info(" Routing Key %s", self.ROUTING_KEY)
     self._channel.queue_bind(exchange=self.EXCHANGE, queue=queue_name,
                              routing_key=self.ROUTING_KEY)



     self._channel.basic_consume(self.callbackmethod, queue=queue_name, no_ack=True)
